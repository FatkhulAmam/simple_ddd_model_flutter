...
class DioLogger{
  static void onSend(String tag, RequestOptions options){
    ...
    Get.log(
        '$tag - Request Path : [${options.method}] ${options.baseUrl}${options
            .path}');
  ...
  static void onError(String tag, DioError error){
    if (null != error.response) {
      Get.log('$tag - Error Path : [${error.response!.requestOptions
          .method}] ${error.response!.requestOptions.baseUrl}${error.response!
          .requestOptions.path} Request Data : ${error.response!
          .requestOptions.data.toString()}');
      Get.log('$tag - Error statusCode : ${error.response!.statusCode}');
      Get.log('$tag - Error data : ${null != error.response!.data ? error
